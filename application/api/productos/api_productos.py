...
# http://0.0.0.0:8080/api_productos?user_hash=12345&action=update&id_productos=1&product=nuevo&description=nueva&stock=10&purchase_price=1&price_sale=3&product_image=default.jpg
    def update(self, id_productos, producto,existencias,precio):
        try:
            config.model.edit_productos(id_productos,producto,existencias,precio)
            productos_json = '[{200}]'
            web.header('Content-Type', 'application/json')
            return json.dumps(productos_json)
        except Exception as e:
        ...
        try:
            ...
            if user_hash == '12345':
                if action is None:
                    ...
                    return self.update(id_productos, producto,existencias,precio)
            else:
                raise web.seeother('/404')
        except Exception as e:
